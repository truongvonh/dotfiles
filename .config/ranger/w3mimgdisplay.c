...
void DrawImage(char *buf, int redraw) {
  char *p = buf;
  ...
  if (n >= maxImage) {
    ...
    if (maxImage > MAX_IMAGE)
      ...
      maxImage = n + 1;
    imageBuf =
        (W3MImage *)realloc((void *)imageBuf, sizeof(W3MImage) * maxImage);
    for (; i < maxImage; i++)
...
void ClearImage(char *buf) {
  ...
  w_op->clear(w_op, x + offset_x - w_op->clear_margin,
              y + offset_y - w_op->clear_margin, w + w_op->clear_margin * 2,
